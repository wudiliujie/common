...
func (client *WSClient) dial() *websocket.Conn {
	for {
		...
		if err == nil || client.closeFlag || client.AutoReconnect == false {
			return conn
		}
...
reconnect:
	...
	agent := client.NewAgent(wsConn, tag)
	agent.SetAutoReconnect(client.AutoReconnect)
	agent.Run()
	...
	client.Unlock()
	agent.OnClose()
	log.Debug("client AutoReconnect:%v", agent.GetAutoReconnect())
	if agent.GetAutoReconnect() {
		time.Sleep(client.ConnectInterval)
		goto reconnect
	}
}
